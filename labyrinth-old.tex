...
\end{equation}% In the next section %\S\ref{sec:quantum}
...
By logic structure we mean that $\vect{F}$ would act like a knowledge base $\KB$ containing a large number of logic \textbf{rules}, as in the setting of classical logic-based AI.
...
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.6]{cylindric-relation.png}}}
\quad \quad \quad
\vcenter{\hbox{\includegraphics[scale=0.6]{cylindric-relation-intersection.png}}}
\end{equation}
...
\end{equation}
We are trying to use neural networks to approximate such functions (\textit{ie}, these geometric shapes).  Since NNs are universal function approximators, they can in principle achieve that.  There is also \textit{empirical} evidence that NNs can well-approximate logical maps, because the \textit{symbolic} matching and substitution of logic variables is very similar to what occurs in \textit{machine translation} between natural languages;  And we know that deep learning is fairly successful at the latter task.
...
\begin{equation}
\label{eqn:action}
\overbrace{ \A^1_1 \A^1_2 \A^1_3 \! \wedge \! \underbrace{\A^2_1 \A^2_2 \A^2_3}_{\makebox[0pt]{each literal made of $m$ atomic concepts, $m = 3$ here}}  \! \wedge \! .... \! \wedge \! \A^k_1 \A^k_2 \A^k_3}^{\mbox{conjunction of $k$ literal propositions}} \quad \Rightarrow \quad \A^0_1 \A^0_2 \A^0_3
\end{equation}
where an atomic concept, or just \textbf{atom}, can be roughly understood as a \textbf{word vector} as in Word2Vec.  Each $\A \in \mathbb{R}^d$ , where $d$ is the dimension needed to represent a single word vector or atom.
...
\begin{equation}
\A^1_1 \A^1_2 \A^1_3 \! \wedge \! \A^2_1 \A^2_2 \A^2_3 \! \wedge \! .... \! \wedge \! \A^k_1 \A^k_2 \A^k_3
\end{equation}
...
\begin{equation}
P(\mbox{action } | \mbox{ state}) := \pi(\A_1 \A_2 \A_3 \; | \; \vect{x}).
\end{equation}
...
\subsection{Commutative / symmetric neural networks}
...
\end{equation}
If we want to use a neural network to model the deduction operator $\vdash: \mathbb{P}^k \rightarrow \mathbb{P}$, where $\mathbb{P}$ is the space of literal propositions, then this function must be \textbf{symmetric} in its input arguments.

A simple fact:  If $\vect{F}(\vect{p},\vect{q})$ is any function, then
\begin{equation}
\vect{F}(\vect{p},\vect{q}) + \vect{F}(\vect{q},\vect{p}) \quad \mbox{or} \quad \vect{F}(\vect{p},\vect{q}) \cdot \vect{F}(\vect{q},\vect{p})
\end{equation}
would be symmetric functions in $(\vect{p},\vect{q})$.  This can be easily extended to $\mathbb{P}^k$.  We will use the additive method.

The \textbf{back-propagation} algorithm can be easily adapted to symmetric NNs.  The gradient of the error, $\nabla \epsilon$, is calculated as usual, in which the key step involves:
\begin{equation}
\nabla \vect{F}_{\mathrm{sym}}(\vect{x}) = \nabla \sum_{\sigma \in \mathfrak{S}_k} \vect{F}(\sigma \cdot \vect{x})
\end{equation}
where $\mathfrak{S}_k$ is the symmetric group of $k$ elements, and $\vect{x} = \vect{p}_1 \wedge \vect{p}_2 \wedge ... \vect{p}_k$.
...
For our purpose, I think the \textbf{Gaussian kernel} (\textit{ie}, radial basis function) method would be very effective.  The true probability distribution is approximated by:
\begin{equation}
...
For each state $x$, our NN should output a probabilistic \textit{choice} of $c$ actions.  So we only need to maintain $c$ ``peaks'' given by Gaussian kernels.  Each peak is determined by its mean (a vector) and variance (a scalar).  We fix the variance globally as the parameter $h$.  So our NN, ie, $Q$-function, would be of the form:
\begin{equation}
Q: \mathbb{X} \rightarrow \mathbb{A}^c
\end{equation}
where each $\A \in \mathbb{A}$ is of the form $\A_1 \A_2 \A_3$ and is of size $\mathbb{R}^{3d}$, as explained above.  Thus our NN is of the from:
\begin{eqnarray}
Q: \mathbb{X} &\rightarrow& \mathbb{A}^c \nonumber \\
= (\mathbb{R}^{3d})^k &\rightarrow& (\mathbb{R}^{3d})^c \nonumber \\
= \mathbb{R}^{3dk} &\rightarrow& \mathbb{R}^{3dc} 
\end{eqnarray}

\subsection{Policy gradient method}

In the policy-gradient method, the policy $\pi(\vect{a} | \vect{x})$ is expressed as a function parametrized by $\Theta$.  The policy is updated via the rule:
\begin{equation}
\Theta \stackrel{+}{=} \eta \; \nabla_{\Theta} J
\end{equation}
where $\eta$ is the \textbf{learning rate}, $J$ is the objective function, which is the expectation of the total reward $R$ along a trajectory $\tau$:
\begin{equation}
J = \underset{\tau}{\mathbb{E}}[ \; R(\tau) \;] .
\end{equation}
The gradient of $J$ can be derived to this formula:
\begin{equation}
\nabla_{\Theta} J = \nabla_{\Theta} \underset{\tau}{\mathbb{E}}[ \; R(\tau) \;] = \underset{\tau}{\mathbb{E}}[ \; \nabla_{\Theta} \sum_t \log \pi(\vect{a}_t | \vect{x}_t; \Theta) R(\tau) \;] .
\end{equation}
...
\item \textbf{Memory:}  In this minimal architecture there is no episodic memory, this will be dealt with in a later version. %\textit{The structure of memory} \cite{YanMemory}.
\end{itemize}

...
\newcommand*\bbTheta{\vcenter{\hbox{\includegraphics{../bbTheta.png}}}}
...
The transition function $\vect{F}$ appearing in (\ref{eqn0}) is ``free'' without further restrictions.  The learning of $\vect{F}$ may be slow without further \textbf{induction bias}, \textit{cf} the ``no free lunch'' theorem \cite{Wikipedia-no-free-lunch}.  But we know that the transition function is analogous to $\vdash$, the logic consequence or entailment operator.  So we want to impose this logic structure on $\vect{F}$.
...
\begin{itemize}
	...
	\end{equation}
	(Now the neighborhood of $\vect{x}_{t+1}$ is a subset $\subset \bbTheta$ which coincides with $\mathbb{X}$ and replaces it)
...
As of this writing I have settled on the ``carousel'' solution:  All the propositions in working memory will enter into a loop, and the reasoning operator \uline{acts on a hidden state $\vect{H} = {\color{green}\bullet}$ and one proposition $\mathsf{P}_i = {\color{red}\bullet}$ at a time}:
\begin{equation}
\label{eqn:carousel-diagram}
...
I believe in the maxim: \textit{Whatever can be done in time can be done in space}.  The diagram (\ref{eqn:carousel-diagram}), when unfolded in time, can be expressed in this functional form:
\begin{equation}
F(\mathsf{P}_0, ..., \mathsf{P}_n) = f(\mathsf{P}_n, f(\mathsf{P}_{n-1}, f(..., f(\mathsf{P}_0, \emptyset)))) .
\end{equation}

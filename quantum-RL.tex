...
\begin{enumerate}
	\item Learning the transition function $\vdash$ or $\squareF: \vect{x} \mapsto \vect{x}'$.  $\squareF$ represents the \textbf{knowledge} that constrains thinking.  In other words, the learning of $\squareF$ is the learning of ``static'' knowledge.
...
In our architecture, $\squareF$ can implemented as a simple feed-forward neural network (where ``deep'' simply means ``many layers''):
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.7]{genifer-model-00.png}}}
% the SVG file is in latex/AGI-16/
\end{equation}

In traditional reinforcement learning (left view), the system chooses an action $\vect{a}$, and the transition function $\squareF$ gives the probability of reaching each state $\vect{x}$ given action $\vect{a}$.  In our model (right view), all possible cognitive states are potentially \textbf{reachable} from any other state, and therefore the action $\vect{a}$ coincides with the next state $\vect{x}'$.
\begin{equation}
...
The cognitive state is a vector $\vect{x} \in \mathbb{X}$ where $\mathbb{X}$ is the space of all possible cognitive states, the reasoning operator $\vdash$ or $\squareF$ is an \textbf{endomorphism} (an \textbf{iterative map}) $\mathbb{X} \rightarrow \mathbb{X}$.

Mathematically this is a \emp{dynamical system} that can be defined by:
\begin{eqnarray}
\boxed{\mbox{discrete time}} \quad \quad \vect{x}_{t+1} & = & \vect{\squaref}(\vect{x}_t) \label{eqn0}\\
...
where $\vect{f}$ and $\vect{\squaref}$ are different but related
\footnote{They are related by: $\vect{x}(t + 1) = \vect{x}(t) + \squaref(\vect{x}(t))$, $\displaystyle \vect{x}^{-1}(\vect{x}(t)) = t =  \int^{\vect{x}_t}_{\vect{x}_0} \frac{d\vect{x}}{\vect{f}(\vect{x}(t))}$, and $\displaystyle \vect{f}(\vect{x}) = \frac{1}{(\vect{x}^{-1})'(\vect{x}(t))}$, where $()'$ denotes derivative w.r.t. $\vect{x}$.  So we can just solve the functional equation $\vect{x}^{-1}(\vect{x} + \squaref(\vect{x})) - \vect{x}^{-1}(\vect{x}) = 1$. See \cite{Dolotin2007} \S8.2.3.
}.

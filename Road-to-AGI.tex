...
\begin{itemize}
	...
	\item 如果限制 状态 $\vec{x}$ = working memory 只有 10 个命题，$\vec{x}$ 的空间是 $\mathbb{P}^{10}/\sim$ 其中 $\sim$ 是对称群 $\mathfrak{S}_{10}$ 的等价关系。 换句话说 $ 2^{\mathbb{P}} \cong \prod_{n=0}^{\infty} \; \mathbb{P}^n / \mathfrak{S}_n $
...
\begin{itemize}
	...
	\item 逻辑的要求略有不同，不是「同类映射到同类」，而是要在庞大的 logic rules 空间中找到适用(applicable)的 rule
	
	\item 隐状态 $s_t$ 代表 ``search state''，注意力 的目的是「选择」$s_t$ 所需要的那些命题，交给 解码器
